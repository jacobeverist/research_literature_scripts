...
\titleformat*{\section}{\LARGE\bfseries\itshape\sffamily}
\titleformat*{\subsection}{\Large\bfseries\itshape\sffamily}
\titleformat*{\subsubsection}{\large\bfseries\itshape\sffamily}
...
\IfFileExists{microtype.sty}{% use microtype if available
  \usepackage[$for(microtypeoptions)$$it$$sep$,$endfor$]{microtype}
...
\ifPDFTeX % if pdftex
\textsf{* $for(correspondence_list)$\emph{$it$}$sep$\quad\&\quad$endfor$}
\else
\textsf{\fixfontB{✉︎} $for(correspondence_list)$\emph{$it$}$sep$\quad\&\quad$endfor$}
\fi
$endif$
